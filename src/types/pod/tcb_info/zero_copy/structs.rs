...
impl<'a> TcbInfoZeroCopy<'a> {
    ...
    pub fn from_bytes(bytes: &'a [u8]) -> Result<Self, ZeroCopyError> {
        ...
        let tdx_module_identities_section_payload = main_payload.get(current_offset..current_offset + identities_len)
            .ok_or(ZeroCopyError::InvalidSliceLength)?;
        current_offset += identities_len;

        let levels_len = header.tcb_levels_total_payload_len as usize;
        let tcb_levels_section_payload = main_payload.get(current_offset..current_offset + levels_len)
            .ok_or(ZeroCopyError::InvalidSliceLength)?;
    ...
    pub fn tdx_module(&self) -> Option<TdxModulePodDataZeroCopy<'a>> {
        self.tdx_module_data_payload.map(TdxModulePodDataZeroCopy::new)
    }
...
impl<'a> TcbComponentZeroCopy<'a> {
    ...
    pub fn new(header: &'a TcbComponentHeader, payload: &'a [u8]) -> Result<Self, ZeroCopyError> {
        ...
        if cat_len.checked_add(comp_type_len).ok_or(ZeroCopyError::InvalidOffset)? > payload.len() {
            return Err(ZeroCopyError::InvalidSliceLength);
        }
        Ok(Self {
            header,
...
impl<'a> TdxTcbLevelZeroCopy<'a> {
    ...
    pub fn new(header: &'a TdxTcbLevelHeader, payload: &'a [u8]) -> Result<Self, ZeroCopyError> {
        ...
        if lengths_len.checked_add(data_len).ok_or(ZeroCopyError::InvalidOffset)? > payload.len() {
            return Err(ZeroCopyError::InvalidSliceLength);
        }
        Ok(Self {
            header,
...
impl<'a> TdxModuleIdentityZeroCopy<'a> {
    ...
    pub fn new(header: &'a TdxModuleIdentityHeader, payload: &'a [u8]) -> Result<Self, ZeroCopyError> {
        ...
        if offset.checked_add(tcb_levels_len).ok_or(ZeroCopyError::InvalidOffset)? > payload.len() {
            return Err(ZeroCopyError::InvalidSliceLength);
        }
    ...
    pub fn tcb_levels(&self) -> TdxTcbLevelIter<'a> {
        TdxTcbLevelIter::new(self.tcb_levels_section_payload, self.header.tcb_levels_count)
    }
}
...
impl<'a> TcbLevelZeroCopy<'a> {
    ...
    pub fn new(header: &'a TcbLevelHeader, payload: &'a [u8]) -> Result<Self, ZeroCopyError> {
        ...
        let sgx_payload = payload.get(current_offset..current_offset + sgx_len)
            .ok_or(ZeroCopyError::InvalidSliceLength)?;
        ...
        let tdx_payload = if header.tdx_tcb_components_present == 1 {
            ...
            let slice = payload.get(current_offset..current_offset + tdx_len)
                .ok_or(ZeroCopyError::InvalidSliceLength)?;
        ...
        let adv_lengths_payload = payload.get(current_offset..current_offset + adv_lengths_len)
            .ok_or(ZeroCopyError::InvalidSliceLength)?;
        current_offset += adv_lengths_len;

        let adv_data_len = header.advisory_ids_data_total_len as usize;
        let adv_data_payload = payload.get(current_offset..current_offset + adv_data_len)
            .ok_or(ZeroCopyError::InvalidSliceLength)?;
        current_offset += adv_data_len;

        if current_offset > payload.len() { return Err(ZeroCopyError::InvalidSliceLength); }
    ...
    pub fn tdx_tcb_components(&self) -> Option<TcbComponentIter<'a>> {
        if self.header.tdx_tcb_components_present == 1 {
            self.tdx_components_strings_payload.map(|payload| {
                TcbComponentIter::new(&self.header.tdx_tcb_components, payload)
            })
        } else {
            None
        }
